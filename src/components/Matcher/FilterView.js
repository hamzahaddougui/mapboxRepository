...
const steps = [
    {
        ...
        header : {
            ...
            subtitle : (<div>Choose as many filters as you like.<br/>You can always change them later</div>)
        }
    },
    {
        key : 2,
...
const FilterView = ({open, setOpen, current, next, previous}) => {
    ...
    return (
        <div className={classes.root} open={open}>
            ...
            <div style={{overflow : "scroll", display : "flex" , justifyContent: "center", alignItems : "center"}}>{steps[current-1].content}</div>
        </div>
    )
}
